...
- **gauge**: single scalar reading against a stated or strongly-implied range, rendered as a ring or dial with optional warn/danger thresholds. Pick gauge when the snippet has ONE numeric value AND meaningful bounds: "memory at 766MB of 8GB", "battery 23%", "latency 340ms p95 (target <500ms)", "score 8.2/10", "queue depth 47 of 100", "coverage 0.71 against the 0.85 target". The range matters — bare scalars without bounds are text. Pick gauge over text when the bounds make the reading meaningful spatially (a dial fills 9% vs 91% communicates instantly what "766MB" alone doesn't). Pick gauge over sparkline / vega when there's only one value, not a series.
...
- **table** — multi-entity × multi-dimension matrix where the cross-product IS the load-bearing claim. Hard minimums: **≥3 entities AND ≥3 dimensions** (both, not either). A 2-column table is dl. A 2-row table is callouts. A 2×2 table is callouts or dl. Reserve table for the case where the reader must scan both rows AND columns to extract the insight. Cap table confidence at 0.55 when the matrix is smaller than 3×3, and at 0.40 when one column is just a name/key column (that's a `dl` in disguise).
...
- **2-column table → dl, not html table.** Specifically: any html cell whose natural shape is `<th>name</th><th>value/description</th>` (e.g., `field × purpose`, `script × what-it-does`, `param × type`, `term × definition`) is a single-entity attribute bag and belongs in `dl`. Hard rule: if you're picking html and the comparison has exactly 2 columns AND one of them is a name/key column, set html_layout=dl. Tables earn their slot only when the cross-product of ≥3 entities × ≥2 dimensions is itself the load-bearing claim.
- **Substrate variety is a quality signal — bias against the dominant defaults.** The dashboard is currently dominated by mermaid (≈39% of cells) and html-table (≈20%). When a snippet's content plausibly fits multiple substrates, deliberately pick the underused one. Concretely: (a) **timeline_ribbon over mermaid `timeline` subtype** when at least one stage has a meaningful current-status (active/failed/skipped) — the ribbon's marker semantics carry that natively; (b) **trajectory over vega scatter** when 2D points are *ordered* and the path between them is the insight; (c) **force_graph over mermaid flowchart** when the topology has cycles, cross-edges, or ≥5 entities forming a mesh rather than a tree; (d) **scene3d over vega** for any quantitative content with 3+ axes or volumetric metaphor; (e) **animated_svg over mermaid** when the snippet emphasizes a process *running* (signal trace, hill-climb, oscillation) rather than its static topology; (f) **callouts / dl / kanban over html-table** for any comparison ≤2 columns or ≤2 entities. The variety of substrate vocabulary is itself part of lucida's value-prop — a dashboard of all-flowcharts reads as a dumber system than a dashboard mixing sankey, mindmap, sequenceDiagram, force_graph, trajectory, and scene3d. When two substrates fit roughly equally on shape grounds, **break ties toward the one with fewer recent picks**.
...
## Example 7 -- comparative (html callouts, not table)
...
Decision: discourse_move=comparative, cell_type=html, confidence=0.74
Reasoning: Two entities (cooperative, competitor), each with a small attribute bag (wage stance, price position, solvency stance). With only 2 rows the table cross-product isn't load-bearing — the reader doesn't need to scan column-by-column; they just want each entity's profile. Pick html_layout=callouts (one card per entity, key facts foregrounded). The prior version of this example routed to table; the tighter ≥3 entities AND ≥3 dimensions rule now demotes it.
