...
- **code**: syntax-highlighted code block. Pick code only when the snippet quotes or names a specific code artifact (function, class, regex, query, CLI command) AND seeing the source clarifies the load-bearing claim that prose alone wouldn't. ≤25 lines fits cleanly; longer than that is a wall and demote to text. Don't pick code when the snippet talks ABOUT code without quoting any -- prose prose-shaped content stays text. Don't pick code for pseudo-code or sketched-syntax that wouldn't actually run -- mermaid for control flow, text for narrative description.
...
- **ascii**: terminal-aesthetic monospace box-drawing diagram (┌─┐ │ │ └─┘ ↗ ↘). Pick ascii when the snippet's structure is a small directory tree, simple pipeline of 3-6 boxes, state stack, queue/buffer, memory layout, or register diagram -- where rectilinear box-drawing carries it clearly and a graphical mermaid diagram would over-render. Cap: ≤6 nodes/boxes. Above that density, mermaid renders better. Don't pick ascii for non-rectilinear relationships (curves, force-graph layouts) or quantitative magnitudes.
