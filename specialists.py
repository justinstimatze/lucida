...
- **No embedded quotes inside flowchart node labels.** Labels are wrapped in double quotes (`X["label"]`), and mermaid's parser does NOT accept `\\"` escape sequences inside — it errors with `Parse error... Expecting 'STR'`. Always rephrase to avoid the quote (e.g. `extractCmd hook` instead of `extractCmd \\"hook\\"`). Do NOT use the HTML entity `#quot;` — older mermaid versions accept it but the current renderer outputs it as the literal string `&quot;` in the SVG, which reads as garbage. If a quote is genuinely load-bearing (a quoted phrase being analyzed), use unicode curly quotes (` " ` U+201C / ` " ` U+201D) directly in the label — mermaid treats them as ordinary characters.
...
- **callout cards** — single big number/value per item, foregrounded. Snippet shape: "X has 47 cells, Y has 152 cells, Z has 8 cells" / status counts / single-stat readouts. Render as `<div class="callouts"><div class="callout"><div class="big">VALUE</div><div class="label">LABEL</div></div>...</div>`. **Each card's `.label` must be distinctive — vary the actual entity / metric / state per card. Do NOT restate shared context across cards.** If 4 of 5 cards would read "X 2025 CHAT INTERFACE TO AI THAT BROWSES" with only the product name swapped, the shared phrase belongs in the cell title or a single dl entry, not duplicated five times — drop the repeated tail or pick `<dl>` so the common attribute name only appears once.
